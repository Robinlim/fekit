...
(function() {
  var connect, http, listenPort, middleware, setupServer, utils;
  ...
  setupServer = function(options) {
    var app;
    app = connect().use(connect.logger('tiny')).use(connect.query()).use(middleware.rewriteObsoleteUrl(options)).use(middleware.rewriteRule(options)).use(connect.bodyParser()).use(middleware.velocity(options)).use(middleware.fekit(options)).use(connect["static"](options.cwd, {
      hidden: true,
      redirect: true
